...
export async function POST(request) {
  ...
  try {
    ...
    const tweetPayload = { text: content.trim() };

    // Add community_id if posting to a community
    if (communityId) {
      tweetPayload.community_id = communityId;
      tweetPayload.share_with_followers = shareWithFollowers || false;
      console.log(`Posting to community: ${communityId}, share_with_followers: ${shareWithFollowers}`);
    }
    ...
    if (!tweetResponse.ok) {
      ...
      return NextResponse.json({ 
        error: tweetData.detail || tweetData.title || tweetData.errors?.[0]?.message || 'Failed to post to X' 
    ...
    let tweetUrl = `https://x.com/${account.platform_username}/status/${tweetId}`;
    if (communityId) {
      tweetUrl = `https://x.com/i/communities/${communityId}/post/${tweetId}`;
    }
    ...
    if (postId) {
      ...
      await supabase
        ...
        .update({
          ...
          share_with_followers: shareWithFollowers || false,
        })
        .eq('id', postId);
    } else {
    ...
    return NextResponse.json({
      success: true,
      message: communityId ? 'Posted to community!' : 'Posted to X!',
      tweetId: tweetId,
      url: tweetUrl,
